      ...
      <section class="content-container bg-white">
        ...
        <div class="switch-btns">
          <el-button
            ...
            @click="getPendingList('pending', { page: 1 })"
            >进行中</el-button
          >
          <el-button
            class="normal-btn"
            :class="gradeListType === 'end' ? 'active-btn' : ''"
            @click="getEndList('end', { page: 1 })"
            >已结束</el-button
          >
        </div>
        <hr />
...
export default {
  ...
  components: {
    ...
    finished: () =>
      import("@/components/modules/myculture/finished/lowerPlus/index.vue")
  },
  methods: {
    getLevelText(num) {
    ...
    handleCurrentChange(val) {
      ...
      if (this.gradeListType === "pending") {
        this.getPendingList(this.gradeListType, { page: val });
      } else {
        this.getEndList(this.gradeListType, { page: val });
      }
      // this.refreshList({
      //   page: val
      // });
    ...
    refreshList(data) {
      return getMyDownMembersCultureList().then(res => {
  ...
  created() {
    this.getPendingList(this.gradeListType, { page: 1 });
