  ...
  <div>
    ...
    <section  class="progress-header" align="center">
      <el-row>
        ...
        <el-col :span="6">
          <el-button :disabled="!Allsubmit_action" type="primary"  @click="Allsubmit_step1">整体提交</el-button>
          <el-popover
          placement="bottom"
          width="270"
          trigger="click">
          ...
          <template>
              <el-table :data="reviewData" height="250">
                <el-table-column width="100" property="created_at" label="日期"></el-table-column>
                <el-table-column width="100" property="name" label="姓名"></el-table-column>
                <el-table-column width="80" property="type_text" label="状态"></el-table-column>
              </el-table>
            </template>
            <el-button style="margin-left:10px" slot="reference">查看审批记录</el-button>
        ...
        <el-row type="flex" :gutter="20" align="top">
          <el-col>
            <el-table :data="tableData" stripe style="width: 100%" :header-cell-style="{background:'#eef1f6'}">
              ...
              >
                <template slot-scope="scope">
                  <el-tag v-if="scope.row.score_level == 'A' || scope.row.score_level == 'S' " class="status-tag top-style">
                    <span class="top-style-text">{{scope.row.label_name}}</span>
                  </el-tag>
                  <el-tag v-else-if="scope.row.score_level == 'B' " class="status-tag bplus-style">
                    <span class="bplus-style-text">{{scope.row.label_name}}</span>
                  </el-tag>
                  <el-tag v-else="scope.row.score_level == 'C' " class="status-tag other-style">
              ...
              <el-table-column prop="ops" label="操作">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.operate_status == 1"
                    ...
                    size="small"
                    >修改评分</el-button>
                    <el-button v-if="scope.row.operate_status == 4"
                    @click="goDetail(scope.row)"
                    type="text"
                    size="small"
                    >处理申诉</el-button>
...
import { getTeamList, highLevelAllSubmit, highLevelReview } from "@/constants/API";
...
export default {
  ...
  methods: {
    ...
    Allsubmit_step1() {
        ...
        this.$confirm('您已评完完成所有直属下级的评分, 请整体检查分布情况，并提交至隔级审核 ','提示', {
          ...
          callback: action => {
            ...
            if( top_Diff >= 0 && b_plus_diff >= 0 && b_diff >= 0 && b_minus_diff >= 0 && cd_diff >= 0 ){//可以直接提交
              this.Allsubmit_step3()
            }else{
              let tip_html =`<p>不符合标签分布要求，如继续提交，将处罚邮件至一级部门负责人和HRBP，是否确认继续</p>\
                       <p>分布结果检查 :</p>\
                       <p style='${ top_Diff >= 0 ? 'display:none' : null }'> <span style='color: #EB0C00;margin-left:90px'>\
                        ${overview[0].name}总人数超出${Math.abs(top_Diff)}人</span></p>\
                       ...
                       <p style='${ b_minus_diff >= 0 ? 'display:none' : null }'> <span style='color: #EB0C00;margin-left:90px'>\
                        ${overview[2].child[0].name}总人数超出${Math.abs(b_minus_diff)}人</span></p>\
                       <p style='${ cd_diff >= 0 ? 'display:none' : null }'> <span style='color: #EB0C00;margin-left:90px'>\
                        ${overview[2].child[1].name}总人数超出${Math.abs(cd_diff)}人</span></p>`
    ...
    Allsubmit_step3() {
        const h = this.$createElement;
        this.$confirm("<p>是否确认提交至隔级审核</p>\
                        <p>分布结果检查 : <span style='color: #EB0C00'> 全部符合23221分布比例要求</span></p>", '提示', {
          dangerouslyUseHTMLString: true,
          confirmButtonText: '提交',
          callback: action => {
            console.log(123)
